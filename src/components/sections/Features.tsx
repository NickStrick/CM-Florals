            ...
            return (
              <AnimatedSection key={i} delay={i * 0.08}>
                <div className={`p-7 ${ink ? 'card-ink' : 'card'}`}>
                  ...
                  <div className={`${ink ? 'text-[var(--text-1)]/90' : 'text-[var(--text-1)]'} text-2xl font-bold mb-2"`}>{f.title}</div>
                  {f.body ? <p className={`${ink ? 'text-[var(--text-1)]/90' : 'text-[var(--text-1)]'}`}>{f.body}</p> : null}
                  <div className="mt-6">
